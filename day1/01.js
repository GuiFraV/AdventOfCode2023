...
const first = (arr) => {
    ...
    // Je push dans l'obj les données
    for(let i = 0 ; i < adOfC.length; i++){
        a.push(adOfC[i]);
    }

    for(let i = 0; i < adOfC.length; i++){
        let num = a[i].match(/\d/g);
        b.push(num);
    ...
    for(let i = 0 ; i < adOfC.length; i++){
        ...
        c.push(parseInt(b[i].join('')));

    }

    d.push(c.join('').match(/.{1,2}/g));
...
console.log(three(adOfC)); 


// 1. Je stocke les chiffres dans une variables
    // - Je créer une variable contenant un obj vide.
    // - Je boucle dans mon array
    // - Je regarde si dans mon string il y a un numéro
        // - je fais un regex pour filtrer les numéros
    // - Si oui, j'envoi le chiffre dans l'obj
    // - Si non, je passe à la suivante
// 2. Je fais la somme des chiffres
// 3. Je donne le résultat